...
class ArcticEngine:
    ...
    def __init__(self):
        ...
        self.scaled_game_surf: Surface = None
        self.scaled_game_surf_dim: tuple[int, int] = None
        ...
        if self.unscaled_game_surf == None or surf_resized  or self.__game_scale_updated:
            ...
            if all(i >= 1 for i in surf_dim):
                ...
                self.unscaled_game_surf_dim = (
                    surf_dim[0] / self.__game_scale, 
                    surf_dim[1] / self.__game_scale)
    ...
    def __set_scaled_game_surf(self, surf_resized: bool, surf_dim: tuple[int, int]):
        ...
        if self.scaled_game_surf == None or surf_resized or self.__game_scale_updated:
                self.scaled_game_surf = transform.scale_by(
                    self.unscaled_game_surf, self.__game_scale)
                
                self.scaled_game_surf = transform.scale(self.unscaled_game_surf, (
                    self.unscaled_game_surf_dim[0] * self.__game_scale,
                    self.unscaled_game_surf_dim[1] * self.__game_scale
                ))
                
                self.scaled_game_surf_dim = surf_dim
                
                print(self.__game_scale, self.unscaled_game_surf_dim, self.scaled_game_surf_dim)
                
                self.__game_scale_updated = False


    def get_game_surf(self, surf_resized: bool, surf_dim: tuple[int, int]) -> Surface:
        ...
        self.unscaled_game_surf.blit(self.map_surf, (
            self.unscaled_game_surf_dim[0]/2 -self.map_surf_dim[0]/2,
            self.unscaled_game_surf_dim[1]/2 -self.map_surf_dim[1]/2,
        ))
